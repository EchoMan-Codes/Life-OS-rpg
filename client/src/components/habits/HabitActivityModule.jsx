...
/**
 * HabitActivityModule — Active momentum telemetry and recent scoring audit stream.
 */
export function HabitActivityModule({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      {/* Left 1/3: Active Momentum Column */}
      <div className="rounded-2xl border border-glass-border bg-obsidian-900/80 backdrop-blur-xl p-5 shadow-glass flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-base font-bold text-ink mb-1.5">Streak Amplification</h3>
        ...
        <div className="mt-4 space-y-2.5">
          {highStreakHabits.length > 0 ? (
            highStreakHabits.slice(0, 3).map((habit) => (
              <div
                key={habit.id}
                className="flex items-center justify-between p-2.5 rounded-xl bg-obsidian-950/70 border border-amber-500/20"
              >
                ...
                <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 font-mono font-bold text-xs shrink-0">
            ...
            <div className="p-4 rounded-xl bg-obsidian-950/40 border border-dashed border-glass-border text-center">
      ...
      {/* Right 2/3: Real Activity Stream */}
      <div className="lg:col-span-2 rounded-2xl border border-glass-border bg-obsidian-900/80 backdrop-blur-xl p-5 shadow-glass">
        ...
        {isLoading ? (
          ...
          <div className="py-8 text-center rounded-xl bg-obsidian-950/40 border border-dashed border-glass-border">
          ...
          <div className="space-y-2">
            {recentLogs.map((log) => {
              ...
              return (
                <div
                  key={log.id}
                  className="flex items-center justify-between gap-3 p-2.5 sm:p-3 rounded-xl bg-obsidian-950/60 border border-glass-border/60 hover:border-glass-border transition-colors"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-ink truncate flex items-center gap-1.5">
                        ...
                        {log.isArchived && (
                          <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-obsidian-800 text-ink-muted border border-glass-border">
                            Archived
                          </span>
                        )}
                      </p>
                  ...
                  <div className="flex items-center gap-2 shrink-0 font-mono text-xs">
                    {isPos ? (
                      ...
                      <span className="px-2 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 font-bold flex items-center gap-1">
